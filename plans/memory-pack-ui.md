...
- [ ] Move settings persistence from `chrome.storage.sync` to `chrome.storage.local`.
- [ ] Add defaults for memory pack settings:
  - enabled,
  - bridge base URL,
  - auth mode,
  ...
  - class filters,
  - insert behavior.
- [ ] Do not include token or memory text in history signatures.
- [ ] Add one-time migration from existing sync settings when present.
...
- [ ] Add background message handlers:
  ...
  - `INSERT_MEMORY_PACK`
  - `COPY_MEMORY_PACK`
- [ ] Implement direct loopback fallback using `POST /pack/browser`.
- [ ] Preferred native host path remains planned; fallback token uses `chrome.storage.local`.
- [ ] Redact errors before returning them to popup.

Exit: popup can call health check and preview without logging raw query/token content.

## T3. Popup UI

- [ ] Add tabs: Queue, Memory Pack, Memory Settings.
- [ ] Add Memory Pack controls:
...
- [ ] Add preview list with checkboxes and metadata.
- [ ] Add Memory Settings controls:
  - bridge URL,
  - auth mode,
  - stored token fallback,
  - default options,
...
- [ ] Add content script request for prompt-box text.
- [ ] Detection priority:
...
- [ ] Add clipboard source only after user action.
- [ ] Insert pack with native setter / bubbled input event where required.
- [ ] Replace existing managed block when present.
- [ ] Preserve the typed prompt after the memory block.
- [ ] Copy fallback when no prompt box exists.
